...
case controllers_Application_saveVote1(params) => {
   call(params.fromPath[String]("idSession", None), params.fromPath[String]("idVoter", None), params.fromPath[String]("vote", None), params.fromPath[String]("time", None)) { (idSession, idVoter, vote, time) =>
        invokeHandler(controllers.Application.saveVote(idSession, idVoter, vote, time), HandlerDef(this, "controllers.Application", "saveVote", Seq(classOf[String], classOf[String], classOf[String], classOf[String]),"GET", """""", Routes.prefix + """idSession/$idSession<[^/]+>/idVoter/$idVoter<[^/]+>/vote/$vote<[^/]+>/time/$time<[^/]+>"""))
   }
}
